...
GRABBER_SRC = polipo-cache-grabber.c
GRABBER_OBJS = polipo-cache-grabber.o

polipo$(EXE): $(OBJS) $(POLIPO_OBJS)
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $(OBJS) $(POLIPO_OBJS)  $(MD5LIBS) $(LDLIBS)

polipo-cache-grabber$(EXE): $(OBJS) $(GRABBER_OBJS)
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $(OBJS) $(GRABBER_OBJS) $(MD5LIBS) $(LDLIBS)
...
all: polipo$(EXE) polipo-cache-grabber$(EXE) polipo.info html/index.html localindex.html
...
install.binary: all
	...
	rm -f $(TARGET)$(BINDIR)/polipo-cache-grabber
	cp -f polipo $(TARGET)$(BINDIR)/
	cp -f polipo-cache-grabber $(TARGET)$(BINDIR)/
